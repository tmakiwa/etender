...
if (isset($_POST['addtender'])) {
    ...
    // Validate and move Tender Invitation Attachment
    ...
    if ($invitation_file['error'] === UPLOAD_ERR_OK && 
        $invitation_file['size'] <= $max_file_size && 
        in_array($invitation_file_extension, $allowed_types)) {
    ...
    // Validate and move Tender Document Attachment
    ...
    if ($document_file['error'] === UPLOAD_ERR_OK && 
        $document_file['size'] <= $max_file_size && 
        in_array($document_file_extension, $allowed_types)) {
    ...
    if ($result) {
        echo "<div class='success'>Tender added successfully!</div>";
        header("refresh:3; url=view_tenders.php"); // Redirect after 3 seconds
        exit();
    } else {
        echo "<div class='error'>Failed to add tender. Please try again.</div>";
    }
}
?>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" name="tender_price" class="form-control" required>
                </div>
            </div>
        </div>
